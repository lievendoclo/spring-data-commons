...
However, if a projection contains a property of a `Collection`, `Map` (or one of its subtypes), that projection will always be considered
as an open projection (see `CollectionAwareProjectionFactory`).
